...
-   `src/shortcut-guard.js` - Firefox uniquement, facultatif : « raccourci prioritaire ». Enregistré par `background.js` (`syncShortcutGuard`, `scripting.registerContentScripts`, `document_start`) seulement si l'utilisateur accorde la permission optionnelle `*://*/*` (pages web HTTP(S)/WebSocket, moindre privilège ; `'<all_urls>'` dans une variable déclenche aussi le faux positif Codacy `xss/no-mixed-html`) depuis les options (`optional_host_permissions`, Firefox 128+). Arrête la propagation de la combinaison du raccourci (lue dans `storage.local.executeActionShortcut`, tenue à jour via `commands.getAll`/`onChanged`) sans `preventDefault()`, pour que Firefox exécute le raccourci malgré l'éditeur de la page. Ne jamais y ajouter de lecture ou d'envoi de données : `PRIVACY.md` s'y engage.
